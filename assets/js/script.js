...
function updateQuote(quoteClass) {
	var quoteValue = $("." + quoteClass).val();

	$.post("includes/handlers/ajax/updateQuote.php", { quote: quoteValue, username: userLoggedIn})
	.done(function(response) {
		$("." + quoteClass).nextAll(".message").text(response);
	});
}
