...
//if submit button was pressed
	...
	$target = "../assets/images/artwork/".basename($image);
//$target_file ="https://pinoyplay.online/assets/images/artwork/".basename($musicFile);
	$targetName = "assets/images/artwork/".basename($image);
	//varaibles for song manipulation

	$musicFile=$_FILES['music']['name'];
	$target_file ="../assets/music/".basename($musicFile);
//$target_file ="https://pinoyplay.online/assets/music".basename($musicFile);
	$target_fileName ="assets/music/".basename($musicFile);
	$uploadOk = 1;
	...
	$query3 = mysqli_query($conn,"SELECT * FROM albums WHERE title='$album'");
		...
		if(mysqli_num_rows($query3) == 1) {
			...
			$queryInsertAlbum="INSERT INTO `albums` VALUES ('','$album', '$artist_id', '$genre_id', '$targetName')";
	...
	$query4 = mysqli_query($conn,"SELECT * FROM songs WHERE title='$songName'");
		...
		if(mysqli_num_rows($query4) == 1) {
			//may song na proceed to check genre
				echo '<script type=text/javascript>alert("Song Is Already Present")</script>';
				header("location:index.php");
		}else{

			//eto yung fetch the row id in album
			$sql3 = "SELECT * FROM albums WHERE title='$album'";
			...
			$queryInsertArtist="INSERT INTO `songs` VALUES ('','$songName','$artist_id','$album_id','$genre_id','$duration','$target_fileName','$albumOrder','0')";
			$ress=$conn->query($queryInsertArtist);
				if($ress==TRUE){
					//display that success yung upload ng new album
					echo '<script type=text/javascript>alert("SONG SUCCESSFULLY ADDED !!")</script>';
				}else{
					echo '<script type=text/javascript>alert("SONG FAILED TO ADD !!")</script>';
				}
...
// Check file size
if ($_FILES["music"]["size"] > 8388608) {
    echo '<script type=text/javascript>alert("Sorry, your file is too large.")</script>';
    $uploadOk = 0;
}
// Allow certain file formats
if($MusicFileType != "mp3" && $MusicFileType != "wav" && $MusicFileType != "m4a") {
	echo '<script type=text/javascript>alert("Sorry, only MP3, WAV & M4A files are allowed.")</script>';
    $uploadOk = 0;
}
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["music"]["tmp_name"], $target_file)) {
    	...
    	echo '<script type=text/javascript>alert("Sorry, there was an error uploading your Music file.")</script>';
    }
}
